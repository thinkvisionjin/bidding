function authuser_password_save() {
    ...
    url = 'updaterow_resetpassword_auth_user?Id=' + $('#authuser_password_Id').val();
    var row = {
        originpassword: $('#authuser_origin_password').val(),
...
<tr id='tr_authuser_password'><td class='tbinputtitle'>原始密码:</td><td><input class='tbinput' type='password' id='authuser_origin_password'/></td></tr>\
...
\
    ...
    $('#authuser_origin_password').jqxPasswordInput({ placeHolder: "输入密码:" });
    ...
    $("#authuser_confirm_password").jqxValidator({
        rules: [
            { input: "#authuser_origin_password", message: "不可为空!", action: 'keyup, blur', rule: 'required' },
