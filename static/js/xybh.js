...
function initProtocolCodeGrid() {
	...
	$("#CreationTime_SEARCH").jqxDateTimeInput({
	...
	$("#jqxgrid").jqxGrid(
					{
						...
						renderstatusbar : function(statusbar) {
							...
							deleteButton.click(function(event) {
								var selectedrowindex = $("#jqxgrid").jqxGrid(
										'getselectedrowindex');
							...
							reloadButton
									.click(function(event) {
										...
										$
												.post(
														"/bidding/default/select?table=ProtocolCode",
														searchContent,
														function(result) {
						...
						rendertoolbar : function(toolbar) {
							...
							if (theme != "") {
								exportButton.addClass('jqx-widget-content-'
										+ theme);
								exportButton.addClass('jqx-rc-all-' + theme);
								printButton.addClass('jqx-widget-content-'
										+ theme);
								printButton.addClass('jqx-rc-all-' + theme);
							}
							printButton
									.click(function(event) {
										var gridContent = $("#jqxgrid")
	...
	$("#popupWindow_EDIT").jqxWindow({
		...
		isModal : true,
		autoOpen : false,
		cancelButton : $("#Cancel_EDIT"),
		modalOpacity : 0.01
	});
	$("#Cancel_EDIT").jqxButton({
		theme : theme
	});
	$("#Save_EDIT").jqxButton({
		theme : theme,
	...
	$("#Save_ADD").click(
			function() {
				...
				//******** ADD_ROW_CONTENT_SAVE START*****************//
				var datarow = row;
				$.post("/bidding/default/insert?table=ProtocolCode", datarow,
						function(result) {
							$("#jqxgrid").jqxGrid('addrow', null, result,
									'first');
						}, 'json');
				$("#popupWindow_ADD").jqxWindow('hide');
			});
