...
class BaseAdapter(with_metaclass(AdapterMeta, ConnectionPool)):
    ...
    def _insert(self, table, fields):
        ...
        if fields:
            keys = u','.join(f.sqlsafe_name for f, v in fields)
            values = u','.join(self.expand(v, f.type) for f, v in fields)
            query = u'INSERT INTO %s(%s) VALUES (%s);' % (table_rname, keys, values);
            print query 
            return query
        else:
    ...
    def insert(self, table, fields):
        ...
        try:
            print '****BASE*****insert' 
#             print '****change query' 
#             query = u'''INSERT INTO Project(ProtocolCodeId,EmployeeId,SourcesOfFundingId,StateId,ProjectName,ManagementStyleId,Assistant,ProjectSourceId,IsDelete,BuyerId,CreationDate,ProjectCodeId,ProjectTypeId) VALUES ('66','1','1','1','娴嬭瘯椤圭洰','1','1','1','0','1','2016-04-27 00:00:00','8','0');'''
            print query
            print '****BASE*****insert' 
            self.execute(query)
        except Exception:
...
#         print expression,field_type
        if isinstance(expression, Field):
            ...
            rv = u','.join(self.represent(item, field_type)
...
#         if PY2 and isinstance(obj, unicode):#fix bug
